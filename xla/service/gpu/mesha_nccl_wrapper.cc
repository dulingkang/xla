...
// This file implements nccl apis for mesha to use.
...
namespace mesha {
namespace {
void AddCallBackReleasingBuffer(se::Stream *stream, py::handle &buf_obj) {
  // Holding the shared ptr of the buffer because we use it in an unsafe way.
  // This prevents XLA from freeing the buffer.
  const std::shared_ptr<PjRtBuffer> ifrt_array = py::cast<xla::PyArray>(buf_obj).pjrt_buffers().front();
  ...
  stream->ThenDoHostCallback([ifrt_array]() {});
}
...
Status PyCommGroup::NcclLocalAllGather(const MeshaNcclUid &key,
                                       std::vector<py::handle> buffers,
  ...
  for (py::handle &buf : buffers) {
    ifrt::Array* ifrt_array = py::cast<xla::PyArray>(buf).ifrt_array();
    // PjRtBuffer* pjrt_buffer = IfrtHelpers::pjrt_buffer(ifrt_array);
    pjrt_buffers.push_back(IfrtHelpers::pjrt_buffer(ifrt_array));
  }
...
Status PyCommGroup::NcclBroadcastPartialGPUs(
    const MeshaNcclUid &key, std::vector<py::handle> buffers,
...
Status PyCommGroup::NcclSend(const MeshaNcclUid &key, py::handle buffer,
                             uint start, uint n_elements, int peer_p2p_rank,
                             bool use_default_stream) {
#if XLA_ENABLE_XCCL
  const int device_id = local_ids[key][0];
  ifrt::Array* ifrt_array = py::cast<xla::PyArray>(buffer).ifrt_array();
  PjRtBuffer* pjrt_buffer = IfrtHelpers::pjrt_buffer(ifrt_array);
  TF_RETURN_IF_ERROR(NcclSendImpl(key, pjrt_buffer, start,
...
Status PyCommGroup::NcclRecv(const MeshaNcclUid &key, py::handle buffer,
                             uint start, uint n_elements, int peer_p2p_rank,
                             bool use_default_stream) {
#if XLA_ENABLE_XCCL
  ifrt::Array* ifrt_array = py::cast<xla::PyArray>(buffer).ifrt_array();
  PjRtBuffer* pjrt_buffer = IfrtHelpers::pjrt_buffer(ifrt_array);
  TF_RETURN_IF_ERROR(NcclRecvImpl(key, pjrt_buffer, start,
                                  n_elements, peer_p2p_rank,
                                  use_default_stream));
  return OkStatus();
#else   // XLA_ENABLE_XCCL
...
// Sync functions:
Status PyCommGroup::CommunicatorRecordEvents(const MeshaUuids &uuids,
...
Status PyCommGroup::CommunicatorWaitEvents(const MeshaUuids &uuids,
                                           int num_devices, bool is_send) {
...
void ResetEventContext(std::shared_ptr<PyClient> client) { ResetMeshaEvents(); }
// Other functions
StatusOr<int> GetBufferDeviceId(py::handle buffer) {
  ifrt::Array* ifrt_array = py::cast<xla::PyArray>(buffer).ifrt_array();
  PjRtBuffer* pjrt_buffer = IfrtHelpers::pjrt_buffer(ifrt_array);
  return pjrt_buffer->device()->local_hardware_id();
}
}  // namespace mesha
}  // namespace gpu
}  // namespace xla