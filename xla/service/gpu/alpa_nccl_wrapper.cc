...
Status PyCommGroup::NcclBroadcastPartialGPUs(
    const AlpaNcclUid &key, std::vector<py::handle> buffers,
...
Status PyCommGroup::NcclSend(const AlpaNcclUid &key, py::handle buffer,
...
Status PyCommGroup::NcclRecv(const AlpaNcclUid &key, py::handle buffer,
                             uint start, uint n_elements, int peer_p2p_rank,
                             bool use_default_stream) {
#if XLA_ENABLE_XCCL
  ifrt::Array* ifrt_array = py::cast<xla::PyArray>(buffer).ifrt_array();
