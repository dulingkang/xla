...
using CrossMeshCommInfo = std::pair<std::shared_ptr<CommGroup>, MeshaNcclUid>;
...
Status CommGroup::NcclCreateCommunicators(
    ...
    const std::vector<int> &device_ids, const MeshaNcclUid &nccl_uid_vec) {
#if XLA_ENABLE_XCCL
...
Status CommGroup::NcclDestroyComms(const MeshaNcclUid &nccl_uid_vec) {
#if XLA_ENABLE_XCCL
...
Status CommGroup::NcclLocalAllGatherImpl(
    const MeshaNcclUid &key, std::vector<PjRtBuffer *> buffers,
...
Status CommGroup::NcclSendImpl(const MeshaNcclUid &key, PjRtBuffer *buffer,
...
Status CommGroup::NcclRecvImpl(const MeshaNcclUid &key, PjRtBuffer *buffer,
                               uint start, uint n_elements, int peer_p2p_rank,
                               bool use_default_stream) {
#if XLA_ENABLE_XCCL
  const int device_id = local_ids[key][0];
...
// Other function
NcclComm::Lock CommGroup::AcquireComm(const MeshaNcclUid &key, int device_id) {
...
// Other functions
MeshaNcclUid NcclUidSerialize(ncclUniqueId nccl_uid) {
  MeshaNcclUid nccl_uid_vec(sizeof(nccl_uid.internal), 0);
  memcpy(nccl_uid_vec.data(), &nccl_uid.internal, sizeof(nccl_uid.internal));
  return nccl_uid_vec;
}

ncclUniqueId NcclUidDeserialize(const MeshaNcclUid &nccl_uid_vec) {
...
Status NcclCreateCommunicators2(
    int world_size, const std::vector<int> &device_global_ranks,
    const std::vector<int> &device_ids, const MeshaNcclUid &nccl_uid_vec) {
  int n_devices = device_global_ranks.size();
  CHECK_EQ(n_devices, device_ids.size());
...
StatusOr<MeshaNcclUid> NcclGetUniqueId() {
  ...
  MeshaNcclUid nccl_uid_vec = NcclUidSerialize(id);
  return nccl_uid_vec;
#else   // XLA_ENABLE_XCCL
  return Unimplemented("NCCL support is not available.");
#endif  // XLA_ENABLE_XCCL
