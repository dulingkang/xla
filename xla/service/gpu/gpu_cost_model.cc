...
std::string Group2Str(nb::object tuple) {
  if (tuple.is_none()) {
    return "()";
  }

  nb::tuple replica_groups = nb::cast<nb::tuple>(tuple);
  std::ostringstream os;
  os << "(";
  for (const auto& group : replica_groups) {
    os << "(";
    for (const auto& id : nb::cast<nb::tuple>(group)) {
      os << nb::cast<int64_t>(id) << ",";
    }
    os << "),";
  }
  os << ")";
...
class ProfilingResult {
 public:
  ...
  ProfilingResult(nb::object prof_result) {
    if (!prof_result.is_none()) {
      PyGILState_STATE gstate = PyGILState_Ensure();
      {
        CommDictPyToCpp(
            nb::cast<nb::dict>(prof_result.attr("all_gather_cost_dict")),
            all_gather_cost_dict_);
        CommDictPyToCpp(
            nb::cast<nb::dict>(prof_result.attr("all_reduce_cost_dict")),
            all_reduce_cost_dict_);
        CommDictPyToCpp(
            nb::cast<nb::dict>(prof_result.attr("all_to_all_cost_dict")),
            all_to_all_cost_dict_);
        CommDictPyToCpp(
            nb::cast<nb::dict>(prof_result.attr("reduce_scatter_cost_dict")),
            reduce_scatter_cost_dict_);
        CommDictPyToCpp(nb::cast<nb::dict>(prof_result.attr("dot_cost_dict")),
                        dot_cost_dict_);
      }
 ...
 private:
  ...
  void CommDictPyToCpp(nb::dict py_dict, CommDict& cpp_dict) {
    // the type of py_dict: Dict[Tuple(group, dtype) -> List[Tuple(size, time)]]
    for (auto item : py_dict) {
      nb::tuple tuple_key = nb::cast<nb::tuple>(item.first);
      std::string dtype_str = nb::cast<std::string>(tuple_key[1]);
      PrimitiveType dtype;
      ...
      nb::list list_val = nb::cast<nb::list>(item.second);
      for (const auto x : list_val) {
        nb::tuple tuple_val = nb::cast<nb::tuple>(x);
        cpp_dict[key].push_back(std::make_pair(nb::cast<int64_t>(tuple_val[0]),
                                               nb::cast<double>(tuple_val[1])));
      }
    }
  }
...
double EstimateHloModuleCost(const HloModule* hlo_module) {
  ...
  for (const HloInstruction* ins : entry_computation->instructions()) {
    ...
    if (ins->IsCustomCall(kGemmCallTarget)) {
      ...
      auto config = ins->backend_config<GemmBackendConfig>().ValueOrDie();  // TODO: hhq. error: no member named 'ValueOrDie' in 'absl::StatusOr<xla::gpu::GemmBackendConfig>'
    ...
    if (cost > 0) {
      spmd::StdCerr(verbose) << ins->ToString() << " cost: " << std::fixed
                             << std::setprecision(8) << cost << std::endl;  // TODO: hhq. error: no member named 'StdCerr' in namespace 'xla::spmd'
    }
