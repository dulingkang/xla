...
void SetPassContext(py::dict dict) {
  for (auto item : dict) {
    ...
    if (py::isinstance<py::bool_>(item.second)) {
      ...
      py::str x = py::cast<py::str>(item.second);
      obj = absl::any(py::cast<std::string>(x));
      ...
      if (tuple_val.size() > 0 && py::isinstance<py::int_>(tuple_val[0])) {
      ...
      } else if (tuple_val.size() > 0 && py::isinstance<py::str>(tuple_val[0])) {
        std::vector<std::string> str_vector;
        str_vector.reserve(tuple_val.size());
        for (size_t i = 0; i < tuple_val.size(); ++i) {
          py::str x = py::cast<py::str>(tuple_val[i]);
          str_vector.push_back(py::cast<std::string>(x));
        }
