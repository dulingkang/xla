#ifndef XLA_SERVICE_SPMD_AUTO_SHARDING_STRATEGY_H_
#define XLA_SERVICE_SPMD_AUTO_SHARDING_STRATEGY_H_
...
class ProfilingResult {
 ...
 private:
  ...
  void PyDictToCppDict(nb::dict py_dict,
    ...
    for (auto item : py_dict) {
      ...
      Key key(Group2Str(nb::cast<nb::tuple>(tuple_key[0])),
              nb::cast<std::string>(tuple_key[1]));
  ...
  std::string Group2Str(const nb::tuple& replica_groups) const {
    std::ostringstream os;
    os << "(";
...
#endif  // XLA_SERVICE_SPMD_AUTO_SHARDING_STRATEGY_H_
