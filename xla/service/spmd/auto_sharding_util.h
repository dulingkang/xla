#ifndef XLA_SERVICE_SPMD_AUTO_SHARDING_UTIL_H_
#define XLA_SERVICE_SPMD_AUTO_SHARDING_UTIL_H_
...
// hhq
template <typename T>
std::string ToString(const absl::Span<T>& vector) {
  std::ostringstream os;
  os << "[";
  for (size_t i = 0; i < vector.size(); ++i) {
    os << vector[i];
...
#endif  // XLA_SERVICE_SPMD_AUTO_SHARDING_UTIL_H_
