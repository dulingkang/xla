...
#include "xla/service/spmd/auto_sharding_strategy.h"
...
HloSharding BroadcastSharding(const HloSharding& input_spec,
  ...
  const auto& new_tile_assignment = input_spec.tile_assignment();

  (void)new_tile_assignment.Reshape(target_tile_assignment_dimensions);
...
void RemoveIndivisibleStrategies(std::unique_ptr<StrategyVector>& strategies,
  ...
  for (size_t i = 0; i < strategies->leaf_vector.size(); ++i) {
    ...
    if (!output_spec.IsReplicated()) {
      ...
      const auto& tile_assignment = output_spec.tile_assignment();
...
HloSharding GetReduceScatterOutput(const HloInstruction* ins,
  ...
  if (ins->opcode() == HloOpcode::kDot) {
    ...
    if (strategy.output_sharding.IsReplicated()) {
      ...
      (void)tile_assignment.Reshape({cluster_env.total_devices});
...
//   return ret;
// }
std::vector<int64_t> GetValuesAlongOneDim(const TileAssignment& array,
                                          int dim) {
  std::vector<int64_t> ret;
  std::vector<int64_t> indices(array.num_dimensions(), 0);
...
void FixMixedMeshShapeResharding(HloInstruction* inst, int operand_num,
  ...
  (void)inst->ReplaceOperandWith(operand_num, replace_with);
}
    ...
    for (HloInstruction* ins : computation->MakeInstructionPostOrder()) {
      if (ins->opcode() == HloOpcode::kDot) {
        ...
        if (lhs_space_dims.size() == 0 && rhs_space_dims.size() == 0) {
          ...
          (void)dot->parent()->ReplaceInstruction(dot, new_dot);
          ...
          (void)ins->parent()->ReplaceWithNewInstruction(ins, std::move(new_ins));
        }
      }
    }
  }
