...
/*static*/ PyBuffer::object PyBuffer::Make(
  ...
  nb::object obj = nb::steal<nb::object>(PyBuffer_tp_new(
  ...
  return nb::borrow<PyBuffer::object>(obj);
}

bool PyBuffer::IsPyBuffer(nb::handle handle) {
  return handle.get_type() == PyBuffer::type();
}

/*static*/ PyBuffer* PyBuffer::AsPyBufferUnchecked(nb::handle handle) {
  return &(reinterpret_cast<PyBufferPyObject*>(handle.ptr())->buffer);
}

/*static*/ StatusOr<PyBuffer*> PyBuffer::AsPyBuffer(nb::handle handle) {
  if (!IsPyBuffer(handle)) {
    return InvalidArgument("Expected a DeviceArray, got object of type %s",
                           nb::cast<std::string>(nb::str(handle.get_type())));
  }
  return AsPyBufferUnchecked(handle);
}

nb::handle PyBuffer::AsHandle() {
...
nb::tuple PyBuffer::python_shape() const {
  return SpanToTuple(buffer()->on_device_shape().dimensions());
}

xla::nb_dtype PyBuffer::python_dtype() const {
  PrimitiveType primitive = buffer()->on_device_shape().element_type();
  return PrimitiveTypeToNbDtype(primitive).value();
}

nb_class_ptr<PjRtDevice> PyBuffer::device() const {
  return nb_class_ptr(client_, buffer_->device());
}
...
StatusOr<nb::object> PyBuffer::CopyToDevice(
    const nb_class_ptr<PjRtDevice>& dst_device) const {
  CHECK(dst_device.get() != nullptr);
  auto transfer_guard_formatter = [this, &dst_device] {
    auto shape = nb::cast<std::string>(nb::str(python_shape()));
    auto dtype = nb::cast<std::string>(nb::str(python_dtype()));
...
StatusOr<nb::object> PyBuffer::AsNumPyArray(nb::handle this_obj) {
  ...
  if (buffer_->IsOnCpu()) {
    ...
    hold->buffer = nb::borrow<nb::object>(this_obj);
    void* data = hold->external_reference_hold->OpaqueDeviceMemoryDataPointer();
    nb::capsule hold_capsule(hold.release(),
                             [](void* h) { delete static_cast<Hold*>(h); });
    nb::ndarray array(dtype, shape->dimensions(), ByteStridesForShape(*shape),
                    data, hold_capsule);
  ...
  TF_ASSIGN_OR_RETURN(nb::object array, LiteralToPython(host_value_->value));
  array.attr("flags").attr("writeable") = Py_False;
  return array;
}
...
StatusOr<nb::dict> PyBuffer::CudaArrayInterface() {
  ...
  TF_ASSIGN_OR_RETURN(nb::str typestr,
  ...
  nb::tuple data(2);
  data[0] = nb::int_(absl::bit_cast<std::uintptr_t>(root_ptr));
  data[1] = nb::bool_(true);  // read-only
...
Status PyBuffer::RegisterTypes(nb::module_& m) {
  // We do not use nb::class_ to build Python wrapper objects because
  ...
  {
    nb::str name = nb::str("DeviceArrayBase");
    nb::str qualname = nb::str("DeviceArrayBase");
  ...
  nb::object base_type = nb::borrow<nb::object>(base_type_);
  base_type.attr("__module__") = m.attr("__name__");
  m.attr("DeviceArrayBase") = base_type;

  {
    nb::tuple bases = nb::make_tuple(base_type);
    nb::str name = nb::str("DeviceArray");
    nb::str qualname = nb::str("DeviceArray");
    PyHeapTypeObject* heap_type = reinterpret_cast<PyHeapTypeObject*>(
        PyType_Type.tp_alloc(&PyType_Type, 0));
    // Caution: we must not call any functions that might invoke the GC until
  ...
  nb::object type = nb::borrow<nb::object>(type_);
  ...
  type.attr("__array__") = nb::cpp_function(
      [](PyBuffer::object self, nb::object dtype, nb::object context) {
        nb::object array = ValueOrThrow(self.buf()->AsNumPyArray(self));
        if (!dtype.is_none()) {
          return array.attr("astype")(dtype);
        }
        return array;
      },
      nb::is_method(type), nb::arg("dtype") = nb::none(),
      nb::arg("context") = nb::none());
  type.attr("__array_priority__") =
      property_readonly([](nb::object self) -> int { return 100; });
  type.attr("_device") = property(
      [](PyBuffer::object self) -> nb_class_ptr<PjRtDevice> {
        return nb_class_ptr(self.buf()->client(),
  ...
  type.attr("aval") = property(
      [](PyBuffer::object self) -> nb::object { return self.buf()->GetAval(); },
      [](PyBuffer::object self, nb::object aval) {
  ...
  type.attr("device_buffer") =
      property_readonly([](nb::object self) { return self; });
  type.attr(
      "shape") = property_readonly([](PyBuffer::object self) -> nb::tuple {
  ...
  type.attr("_value") = property_readonly(
      [](PyBuffer::object self) -> StatusOr<nb::object> {
        GlobalPyRefManager()->CollectGarbage();
        return self.buf()->AsNumPyArray(self);
      });
  type.attr("copy_to_device") = nb::cpp_function(
      [](PyBuffer::object self, const nb_class_ptr<PjRtDevice>& dst_device) {
        return self.buf()->CopyToDevice(dst_device);
      },
      nb::is_method(type));
  type.attr("copy_to_remote_device") = nb::cpp_function(
      [](PyBuffer::object self, const nb::bytes serialized_descriptor) {
        // TODO(phawkins): remove the std::string cast after C++17 is required.
        // nb::bytes has a std::string_view cast, but not an absl::string_view
        // cast.
  ...
  type.attr("delete") = nb::cpp_function(
      [](PyBuffer::object self) { self.buf()->Delete(); }, nb::is_method(type));
  type.attr("block_host_until_ready") = nb::cpp_function(
  ...
  type.attr("is_ready") = nb::cpp_function(
      ...
      nb::is_method(type));
  type.attr("is_known_ready") = nb::cpp_function(
  ...
  type.attr("block_until_ready") = nb::cpp_function(
      ...
      nb::is_method(type));
  type.attr("copy_to_host_async") = nb::cpp_function(
      [](PyBuffer::object self) { return self.buf()->CopyToHostAsync(); },
      nb::is_method(type));
  type.attr("xla_shape") = nb::cpp_function(
      [](PyBuffer::object self) { return self.buf()->shape(); },
      nb::is_method(type));
  type.attr("xla_dynamic_shape") = nb::cpp_function(
  ...
  type.attr("device") = nb::cpp_function(
  ...
  type.attr("platform") = nb::cpp_function(
      ...
      nb::is_method(type));
  type.attr("is_deleted") = nb::cpp_function(
  ...
  type.attr("clone") = nb::cpp_function(
      [](PyBuffer::object self) { return self.buf()->Clone(); },
      nb::is_method(type));
  type.attr("__module__") = m.attr("__name__");

  nb::class_<PyShardedBuffer>(m, "ShardedBuffer")
      .def(nb::init(&PyShardedBuffer::CreateFromPyBuffers))
      // .def("__init__",
      //      [](XlaBuilder* self, absl::Span<const PyBuffer::object> py_buffers) {
      //        new (self) PyShardedBuffer::CreateFromPyBuffers(py_buffers);
      //      })
      ...
      .def_prop_ro("dtype", [](const PyShardedBuffer& self) {
        return PrimitiveTypeToNbDtype(self.dtype()).value();
      });

  nb::implicitly_convertible<std::vector<PyBuffer::object>, PyShardedBuffer>();
