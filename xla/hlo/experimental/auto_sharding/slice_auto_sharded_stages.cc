...
std::pair<std::string, std::string> ParseNameAndType(const HloInstruction* ins) {
  ...
  // std::regex pattern(R"(pipeline_marker\[mark_type=(.+) name=(.+)\])");
  // std::smatch matches;
  // std::regex_search(ins->metadata().op_name(), matches, pattern);
  // auto tokens = SplitString(matches[2].str() + "$" + matches[1].str(), '$');
...
std::vector<std::unique_ptr<HloModule>> SliceAutoShardedStagesInternal(
  ...
  {
    py::object submodule =
        py::module::import("mesha.shard_parallel.auto_sharding");  // added by mesha
    py::list stage_names;
