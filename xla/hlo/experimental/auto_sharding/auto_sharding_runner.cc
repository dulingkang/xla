...
Status RunAutoShardingPass(HloModule* hlo_module,
  ...
  layout_insensitive_algsimp_opts.set_enable_dot_strength_reduction(false);  // Added by mesha
  ...
  if (hlo_module->config().use_spmd_partitioning()) {
    ...
    if (num_partitions > 1) {
      ...
      spmd_pipeline.AddPass<DotDecomposer>();  // Added by mesha
      ...
      spmd_simplify.AddPass<TransposeFolding>(
          gpu::CanFoldTransposeOperandIntoDot);  // Added by mesha
      spmd_simplify.AddPass<HloCSE>(
          /*is_layout_sensitive=*/false);  // Added by mesha
